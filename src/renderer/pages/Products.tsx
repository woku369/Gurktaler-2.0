    ...
    <div className="p-8">
      ...
      <div className="space-y-4">
        {filteredTree.map((product) => (
          <div key={product.root.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            {/* Root Product */}
            <div className="p-6 hover:bg-slate-50 transition-colors">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-semibold text-slate-800">{product.root.name}</h3>
                    <span className="text-sm text-slate-500">v{product.root.version}</span>
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[product.root.status as keyof typeof statusColors]}`}>
                      {statusLabels[product.root.status as keyof typeof statusLabels]}
                    </span>
                  </div>
                  {product.root.description && (
                    <p className="text-sm text-slate-600 mb-2">{product.root.description}</p>
                  )}
            ...
            {product.versions.length > 0 && (
              <div className="border-t border-slate-200 bg-slate-50">
                {product.versions.map((version) => (
                  <div key={version.id} className="p-4 pl-12 hover:bg-slate-100 transition-colors border-b border-slate-200 last:border-b-0">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className="text-sm text-slate-500">v{version.version}</span>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[version.status as keyof typeof statusColors]}`}>
                            {statusLabels[version.status as keyof typeof statusLabels]}
                          </span>
                        </div>
                        {version.description && (
                          <p className="text-sm text-slate-600 ml-8 mb-1">{version.description}</p>
                        )}
                        {version.archive_reason && (
                          <p className="text-sm text-red-600 italic ml-8">Archiviert: {version.archive_reason}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <button
